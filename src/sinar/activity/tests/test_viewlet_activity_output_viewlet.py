...
class ViewletIntegrationTest(unittest.TestCase):
    ...
    def test_activity_output_viewlet_is_registered(self):
        view = BrowserView(self.portal['other-document'], self.request)
        manager_name = 'plone.abovecontenttitle'
        alsoProvides(self.request, ISinarActivityLayer)
        ...
        manager.update()
        my_viewlet = [v for v in manager.viewlets if v.__name__ == 'activity-output-viewlet']  # NOQA: E501
        self.assertEqual(len(my_viewlet), 1)

    # XXX would be nice to have this test working:
    # def test_activity_output_viewlet_is_not_available_on_newsitem(self):
